...
clone_div.find('#sendtasks').attr({id: "sendtasks_" + id, onclick: "togtaskreport(id)"});
   // clone_div.find('.onhold_api1').attr({name: "onhold_api_" + id, onchange: "togIntegratedApi(, true)", class: ''});
    ...
    clone_div.find('#assignmnetinfo').attr({id: "assignmnetinfo_" + id});
    clone_div.find('#operationinfo').attr({id: "operationinfo_" + id});
    clone_div.find('#locationinfo').attr({id: "locationinfo_" + id});
    clone_div.find('#createinfo').attr({id: "createinfo_" + id});
    clone_div.find('#updateinfo').attr({id: "updateinfo_" + id});
    clone_div.find('#assetinfo').attr({id: "assetinfo_" + id});
    clone_div.find('#attachmentinfo').attr({id: "attachmentinfo_" + id});
    clone_div.find('#customerinfo').attr({id: "customerinfo_" + id});
...
function createappendUpdateData(name, id) {
    ...
    if (value) {
        ...
        $('#create_' + name + '_data_' + id).append('<div>' + value + '</div>');
...
$(document).on("click", "#reject_value", function () {
 ...
 function togtaskreport(id) { 
     var thenum = id.match(/\d+/g).map(Number);
    $('#sndtsks_'+thenum).toggle(600);
